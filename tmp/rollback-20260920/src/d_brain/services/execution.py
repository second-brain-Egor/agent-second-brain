...
class Execution:
    def __init__(self, project: Path, *, scope='', request='', origin='cli',
        ...
        self.seconds = seconds or positive_env('TASK_TIMEOUT_SECONDS', 600)
        self.max_calls = max_calls or positive_env('TASK_MAX_MODEL_CALLS', 1)
        self.max_steps = max_steps or positive_env('TASK_MAX_STEPS', 24)
    ...
    def begin_call(self):
        with self.lock:
            self.check()
            if self.data['calls'] >= self.max_calls:
    ...
    def observe(self, line: str, backend: str):
        ...
        if identity and identity not in self.seen:
            ...
            # CLI tools that repeatedly wait consume model turns. Stop that loop;
            # the Python supervisor itself waits for exit without invoking a model.
            tool = str(item.get('tool', ''))
            command = str(item.get('command', ''))
            if tool.split('__')[-1] in {'wait', 'write_stdin', 'sleep', 'wait_agent'} or (
                command.startswith(('sleep ', 'watch '))
            ):
                self.wait_steps += 1
                if self.wait_steps > 2:
                    self.limit('Остановлен цикл повторных проверок ожидания.')
