...
from d_brain.services.execution import (
    ...
    final_text, positive_env, run_bounded,
)
...
def _file_lock(path: str) -> Iterator[None]:
    ...
    try:
        deadline = time.monotonic() + positive_env("TASK_LOCK_WAIT_SECONDS", 2)
        while True:
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError("Другая обработка уже выполняется. Новый запуск не поставлен в очередь.")
                time.sleep(0.05)
        yield
    finally:
        try:
...
class AgentProcessor:
    ...
    def _render_session_entry(entry: dict[str, Any]) -> str:
        ...
        text = entry.get("text", "")
        if entry_type == "file":
            from d_brain.services.documents import render_file_entry
...
"""
    ...
    def _build_exec_prompt(
        ...
        *,
        ...
        return (
            ...
            "- Reply in Russian.\n"
            "- Выполняй только текущее поручение. Прошлые незавершённые дела — контекст, "
            "не команда возобновлять их. Уточнение относится к ближайшей теме.\n"
            "- Не вызывай модель, wait, sleep или повторные команды ради ожидания фоновой "
            "работы. Ожиданием и состоянием задачи управляет обычный код бота. "
            "Если отдельный процесс уже запущен, сохрани его идентификатор и прогресс "
            "и сразу верни честный статус. Не выдавай запуск за завершение.\n"
            "- У задачи ограничены время, запуски модели и число шагов. "
            "Сохраняй завершённые этапы в файлах по ходу работы; не начинай "
            "новые старые поручения из истории.\n"
    ...
    def _run_chat(
        ...
        *,
        ...
        Время ограничено; автоматические повторы отключены.
        """
        ...
        return self._run_backend_exec(
            ...
            timeout_sec=timeout_sec or positive_env("TASK_TIMEOUT_SECONDS", 600),
            mode="chat",
        )
...
"""
    ...
    def process_pending(self, today: date | None = None) -> dict[str, Any]:
        """Process all pending daily entries, including historical gaps."""
        execution = CURRENT_EXECUTION.get()
        if execution is None:
            execution = Execution(self.project_path, request="Обработка необработанных записей",
                                  max_calls=positive_env("PROCESS_MAX_MODEL_CALLS", 8),
                                  max_steps=positive_env("PROCESS_MAX_STEPS", 96))
            with execution_context(execution):
                try:
        ...
        with _file_lock(str(lock)):
            ...
            completed: list[str] = []
            processed = 0
            execution.update(pending_days=[day.isoformat() for day in days])
            for day in days:
                execution.check()
                result = self.process_daily(day)
                if "error" in result:
                    # Keep the failed day pending and stop this batch for retry.
                    return {
                        "error": f"{day.isoformat()}: {result['error']}",
                        "processed_entries": processed,
                        "days": completed,
                    }
                processed += int(result.get("processed_entries", 0))
                completed.append(day.isoformat())
                execution.update(completed_days=list(completed))
                text = str(result.get("report", "")).strip()
                if text:
                    reports.append(text)
                # Entries may arrive while this day is being processed. Leave
                # their tail for the next run before continuing to later days.
                if self._daily_unprocessed_tail(day):
                    break
            return {
                "report": "\n\n".join(reports) or "Новых записей для обработки нет.",
                "processed_entries": processed,
                "days": completed,
            }
