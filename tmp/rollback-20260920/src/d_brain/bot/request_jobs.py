...
from d_brain.services.execution import (
    ...
    execution_context, positive_env,
)
...
SHORT_REQUEST = re.compile(r'^(?:дай|пришли|покажи)\s+(?:(?:мне|просто|только)\s+)*(?:ссылку|путь)\b', re.I)
...
class RequestJobs:
    ...
    def status(self, scope):
        ...
        if existing:
            ...
            return f"Задача выполняется. Выполнено шагов: {data['steps']} из {execution.max_steps}.{waiting}"
        if queued:
    ...
    async def __call__(self, handler, event, data):
        ...
        if hasattr(event, 'model_dump_json'):
            if data.get('restored_inbox_key') != key:
                if not self.inbox.accept(key, scope, event.model_dump_json()):
                    return None
        # Preserve every attachment in an album; it must not cancel its siblings.
        ...
        if not attachment:
            ...
            predecessors = tuple(turns.values())
            turns[message.message_id] = asyncio.Event()
        if message.voice:
        ...
        if STATUS.fullmatch(text.strip()):
            ...
            return
        key = f'{scope}:message:{message.message_id}'
        short_request = bool(SHORT_REQUEST.search(text.strip()))
        batch = command == '/process' or text == '⚙️ Обработать'
        execution = Execution(self.settings.vault_path.parent, scope=scope,
                              request=text or '[вложение]', origin='bot',
                              seconds=positive_env('SHORT_REQUEST_TIMEOUT_SECONDS', 90) if short_request else None,
                              max_calls=positive_env('PROCESS_MAX_MODEL_CALLS', 8) if batch else None,
                              max_steps=(positive_env('PROCESS_MAX_STEPS', 96) if batch else
                                         positive_env('SHORT_REQUEST_MAX_STEPS', 8) if short_request else None))
        ...
        task = asyncio.create_task(self.run(handler, event, dict(data), key, execution, predecessors))
    ...
    async def run(self, handler, event, data, key, execution, predecessors=()):
        message = event.message
        with execution_context(execution):
            try:
                # Queue in Python, without another model call or a blocked update handler.
                # Each handler reads the conversation after its predecessor has replied.
                for predecessor in predecessors:
                    await predecessor.wait()
                execution.deadline = time.monotonic() + execution.seconds
                execution.check()
    ...
    async def run_document(self, store, job, processor):
        ...
        execution = Execution(self.settings.vault_path.parent, scope=doc['scope'],
                              request=job['request'], origin='bot', max_calls=2)
