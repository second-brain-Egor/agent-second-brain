...
while IFS= read -r folder; do
  ...
  prompt="Прочитай полностью vault/projects/Nate Herk/AGENTS.md, затем обработай только ролик в папке: ${folder#$PROJECT_DIR/}. Создай полноценный analysis.md строго по правилам проекта. Обязательно используй точные отдельные заголовки второго уровня: «Тема», «Краткое содержание», «Основной вывод», а затем остальные разделы карточки. Проверь ссылки на кадры. После этого аккуратно дополни vault/projects/Nate Herk/summary.md новыми знаниями без повторов. Не трогай другие карточки. Не отвечай пользователю и не отправляй сообщения: это фоновая обработка."
