...
DAILY_SCHEDULE="${DAILY_CRON_SCHEDULE:-30 2 * * *}"
...

{
    ...
    echo "$DAILY_SCHEDULE cd $PROJECT_DIR && /bin/bash $PROJECT_DIR/scripts/process.sh >>$PROJECT_DIR/logs/process.log 2>&1"
...
echo "  $DAILY_SCHEDULE  daily processing"
