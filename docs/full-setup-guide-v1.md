...
| Кнопка | Что делает |
|--------|------------|
| 🛠 Работа | Включает очень высокий уровень мышления (`xhigh`) |
| ⚙️ Обработать | Запускает обработку всех записей |
| 📅 Неделя | Показывает недельный дайджест |
| 💬 Поболтать, обсудить | Включает средний уровень мышления (`medium`) |
| 🤖 Модель | Переключает Claude / Codex |
| 🧠 Claude | Выбирает модель Claude |
