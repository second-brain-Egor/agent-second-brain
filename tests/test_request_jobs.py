...
async def test_old_deadline_setting_does_not_stop_job(jobs,monkeypatch):
    ...
    assert execution.seconds is None
...
async def test_short_link_request_has_no_special_budget(jobs):
    async def work(*args):
        await asyncio.Event().wait()
    await jobs(work,SimpleNamespace(message=message('Дай просто ссылку на папку и всё')), {})
    execution=jobs.active['7:message:1'][0]
    assert execution.seconds is None and execution.max_steps is None
    await jobs.close()
...
async def test_stop_cancels_all_running_messages(jobs):
    started = asyncio.Event()
    handled = []
...
async def test_slow_voice_does_not_delay_later_text(jobs, monkeypatch):
    ...
    await first
    await asyncio.wait_for(asyncio.gather(*list(jobs.tasks)), 1)
    assert handled == ['Уточнение', 'Первое поручение']
...
async def test_later_message_starts_without_waiting(jobs):
    release = asyncio.Event()
