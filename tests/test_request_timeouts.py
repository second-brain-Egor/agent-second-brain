...
def test_routes_use_bounded_structured_cli(processor, tmp_path, backend, route):
    ...
    assert states[0]['seconds'] is None
...
def test_default_execution_has_no_new_limits(tmp_path, monkeypatch):
    for name in ('TASK_TIMEOUT_SECONDS', 'TASK_MAX_MODEL_CALLS', 'TASK_MAX_STEPS'):
    ...
    assert execution.data['calls'] == 100
    assert execution.data['steps'] == 100
    execution.check()


def test_explicit_launch_budget_prevents_retry(tmp_path):
    execution = Execution(tmp_path, max_calls=1)
    execution.begin_call()
    with pytest.raises(ExecutionLimit):
        execution.begin_call()
