---
description: "TimberFrame — project"
type: project
tags: 
source: daily/2026-04-15
created: 2026-04-15
last_accessed: 2026-04-15
relevance: 0.94
tier: active
---

# TimberFrame

Отдельный проект по направлению timberframe.

## Контекст

Это не только мастерская и оборудование, а весь долгий контур: технология, производство, будущие объекты и развитие своего дела.

## Что входит

- мастерская и оборудование для обработки бруса
- геометрия и технология соединений
- баня, беседка и другие будущие объекты
- производственный контур под timberframe

## Связи

- [[projects/timberframe-workshop-setup/README|Timberframe Workshop Setup]]
- [[goals/0-vision-3y|Vision 2025-2027]]
- [[goals/1-yearly-2026|Goals 2026]]

## Литература

- Steve Chappell — «A Timber Framer's Workshop: Joinery, Design & Construction of Traditional Timber Frames». 2026-05-14: на Anna's Archive (.org/.gl/.se) книги не нашлось ни по ISBN, ни по названию; в свободных зеркалах сейчас отсутствует. Варианты: купить бумажный экземпляр (Amazon, Fox Maple Press) или искать в офлайн-библиотеках timber framers guild.
